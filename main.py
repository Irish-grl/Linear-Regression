...
# C950 Package Delivery Project
#
# Program uses three delivery trucks to 'deliver' a specified amount of packages to corresponding addresses within a
...
class Hashing:
    ...
    #c onstructor with initial capacity for table as 10
    # assigns all buckets with an empty list for the hash table
    def __init__(self, initial_capacity = 10):
        self.table = []
        for i in range (initial_capacity):
            self.table.append([])

    # inserts an item into the bucket list
    def insert(self, key, item):
        ...
        # insert the item at the end of the list in the bucket
        key_value = [key, item]
        bucket_list.append(key_value)
        return True

    # searches for matching key for item within the hash table
    ...
    # finds and removes item with matching key value from the bucket list
    def remove(self, key):
        # get the bucket list where key would be
        bucket = hash(key) % len(self.table)
        bucket_list = self.table[bucket]
...
def loadMovieData(fileName, myHash):
    with open(fileName) as movie:
        movieData = csv.reader(movie, delimiter=',')
...
loadMovieData('movie.csv', myHash)

def getMovieData(self):
    print("Best Movies from Hashtable:")
...
def get_shortest_path_city(start_vertex, end_vertex):
    ...
    current_vertex = end_vertex
    while current_vertex is not start_vertex:
        myMovie = Hashing.search(int(current_vertex.label))
...
for v in g.adjacency_list:
    myMovie = Hashing.search(int(v.label))
    if v.pred_vertex is None and v is not vertex_1:
